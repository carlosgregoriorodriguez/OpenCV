...
def filtering(img_butt):
    ...
    if cv2.getTrackbarPos('show_median_blur','config') >= 1:
    ...
    if cv2.getTrackbarPos('show_erode','config') >= 1:
...
def calcContours(image):
    ...
    if cv2.getTrackbarPos('show_canny','config') >= 1:
...
def calcMask(img):
    ...
    cv2.createTrackbar('show_erode','config',0,1,on_filter_trackbar)
    cv2.createTrackbar('show_canny','config',0,1,on_filter_trackbar)
