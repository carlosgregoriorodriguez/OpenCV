...
\notaCarlos{Algunas observaciones globales:
  \begin{itemize}
  \item Tiempo verbal. Ha veces usas el pasado, a veces el presente, a veces el presente continuo... Procura unificar el estilo, lo más fácil es que sea siempre en pasado.
  \item Persona. Aquí ya lo hablamos, lo normal es usar el plural, pero siempre que 
se pueda utiliza el impersonal. Ejemplo:
``Para hacer este reescalado primero voy a tratar de encontrar...'' se puede poner
``Hacer el reescalado consiste en encontrar el rectángulo...''
\item Unifica los términos y la notación. Ejemplo: a veces usas color-check, a veces rectángulo de colores... ; A veces utilizas floodfill a veces cv2.floodfill...
\item Más notación:
``y lo vamos a llamar `A'. A siempre va a ser mayor que el area de la mariposa, que vamos a llamar `a'... '' Para nombrar así se utiliza el modo matemático ``y lo vamos a llamar $A$. $A$ siempre va a ser mayor que el area de la mariposa, que vamos a llamar $a$ ''

\item Trata de explicar todos los conceptos nuevos cuando aparezcan, por ejemplo hablas de histogramas como si todo el mundo supiese lo que és. Como tienes apéndices en las que de una u otra forma aparece toda esta información técnica, haz referencias a ellos.
\item Usa el corrector ortográfico
\item En las secciones en las que explicas un procedimiento que tiene varios pasos y vas explicando a medida que avanzas, termina con un resumen de todas las funciones que has utilizado. Ejemplo: al final de la sección 3.1 ``Encontrando el color-check'', poner algo así como ``En resumen, los pasos a seguir son
  \begin{itemize}
  \item Utilizamos \texttt{cv2.matchTemplate} con parámetros tal y cual ..
  \item Luego nuestro filtro \texttt{floodfillgray.py}....
  \item Y finalmente, para encontrar los contornos....
  \end{itemize}
En alguna sección (creo que 4.2) he visto que ya has hecho algo parecido.
\end{itemize}
}
...
\notaCarlos{Mucho detalle técnico, piensa en que el lector debería entenderlo aunque no quisieras meterte en los detalles} 
...
\notaCarlos{Explicación de lo que se puede encontrar en el documento
  \begin{itemize}
  \item Capítulo 3...
  \item Capítulo 4...
  \item ...
...
\notaCarlos{
Hasta aquí se supone que todo debe ser expresado en lenguaje general, sin tecnicismos.
A partir de aquí van a comenzar los tecnicismos y tenemos que aclarar la nomenclatura que vamos a utilizar 
\begin{itemize}
\item Vamos a nombrar muchas funciones de la librería OpenCV, estas funciones ya están implementadas y algunas de ellas explicadas con detalle en apéndice A... En el texto las nombraremos \texttt{cv2.Canny} (o \textit{cv2.Canny} como veo que has puesto, o como tu quieras, es nomenclatura, pero se suele utilizar más la tipografía \texttt{typewriter}).
\item También nombraremos funciones propias que se han ido implementando a medida que se desarrollaba el proyecto, la nombraremos \texttt{flodfillgray.py} (o como quieras, igual que antes) pues están implementadas en python utilizando la librería opencv. (De estas funciones, las que salgan, quizás se podrían poner en el apéndice de código) 
\end{itemize}
}
...
Vamos a intentar encontrar la posición del color-check encontrando su contorno y para que esto sea más sencillo, hay que intentar hacer que destaque sobre el fondo. 

Empezamos probando con diferentes thresholds aplicados sobre toda la imagen. Utilizando los programas \texttt{test\_threshold.py} y \texttt{adapThres.py} con varias imágenes distintas comprobamos que el threshold adaptativo es el que mejor podría funcionar, pero al empezar a probar, rápidamente nos damos cuenta de que no va a ser así ya que, independientemente de los parámetros que tomemos, algunos de los colores se quedan en blanco y en varias imágenes llegan a hacer que el rectángulo quede dividido en dos partes (véase \textit{Figura \ref{adapThresh}}).
...
Una vez que hemos conseguido separar la tarjeta de color del fondo, el siguiente paso es encontrar el contorno. Para ello utilizamos \texttt{cv2.Canny} seguido de \texttt{cv2.findContours}. Como el color-check es un rectángulo, lo que hacemos para estar seguros de que el contorno seleccionado es el correcto es calcular el rectángulo de área mínima que contiene a cada uno y quedarnos con el que tenga el área y la proporción alto/ancho más parecido al del color-check aislado que habíamos guardado.
...
Una vez que hemos encontrado el rectángulo de colores en cada imagen tenemos que reescalarlas.\\
 Vamos a hacer que todos los color-check tengan el mismo tamaño con una simple regla de tres. Para intentar perder la menor información posible sobre las imágenes, principalmente del color, el nuevo valor de alto y ancho que vamos a elegir es la media de los altos y anchos de cada imagen.

Uniendo todo, obtenemos el programa \texttt{resizeAndWrite.py} que hace lo que deseábamos: encontrar en cada imagen el color-check y utilizando éste, hacer que todas las imágenes queden a la misma escala.  
...
Esta parte del trabajo aparentemente era muy sencilla ya que a simple vista parecía que aplicando un simple threshold o floodfill sería suficiente para lograr lo que deseábamos, pero cuando nos pusimos a trabajar en ello nos dimos cuenta de que no es así. Al contrario de lo que creíamos, debido a problemas con las muestras y a la gran variedad de ejemplares (algunos muy diferentes), aislar la mariposa se convirtió en un gran problema y, por tanto, en el núcleo del trabajo.  
...
El threshold que utilizo es \texttt{THRESH\_BINARY\_INV} que nos deja el fondo negro y la mariposa en blanco, como deseábamos.

 Como una vez hecho esto el contorno es muy desigual, aplicamos \texttt{cv2.erode} otra vez sobre la imagen para que el borde se suavice y de esta forma se eliminen posibles zonas blancas aisladas que haya en el fondo.

Juntando todo obtengo el programa \texttt{calcBinaryImage.py} que se divide en cuatro partes fundamentales:
\begin{itemize}
...
\item Un filtro posterior para eliminar pequeños fallos del \texttt{floodfill} y suavizar los bordes de la máscara.
\end{itemize}
...
Para las comparaciones que se realizan con \texttt{cv2.compareHist} podemos elegir varios métodos. En nuestro caso, hemos elegido el de comparación por correlación y chi cuadrado (CV\_COMP\_CORREL y CV\_COMP\_CHISQR), el primero funciona un poco peor pero más rápido y el segundo al revés, más lento pero un poco mejor. 
...
El programa que realiza todo esto es \texttt{searchByColor.py} y tiene dos pasos principales:
\begin{itemize}
...
Como ya he dicho, estos momentos son invariantes a traslación, escalado y rotación y los vamos a calcular a partir de la imagen binaria obtenida tras aplicar \texttt{calcBinaryImg.py} a las imágenes de la base de datos.
...
Éstos momentos serían muy fáciles de utilizar si tuviéramos un contorno cerrado y continuo para todas las imágenes.Como no hemos conseguido obtener el contorno de dicha manera resulta muy difícil o casi imposible calcular algunos de ellos, ya que, por ejemplo, \texttt{cv2.contourArea} no nos daría el área real del contorno por no estar cerrado.
...
Para poder utilizar este método tenemos que seguir investigando la forma de obtener un contorno continuo y cerrado para todas las imágenes.
 ...
 Como de momento no lo hemos conseguido decidimos crear \texttt{searchBySize\_contour.py} que compara las imágenes por forma utilizando \texttt{cv2.matchShapes} y los momentos 1, 3 y 6 (en 3 en vez de tomar el área del contorno que devuelve la función \texttt{cv2.contourArea} utilizo el momento m00).
...
\section{Contornos}
Muchos de los algoritmos que se utilizan para la detección de bordes están basados en el cálculo del gradiente, es decir, la dirección de máxima variación. Algunos de estos métodos son \texttt{cv2.Sobel}, \texttt{cv2.Laplacian} o \texttt{cv2.Canny}.

En esta sección se explica con más detalle el algoritmo Canny ya que es el más utilizado a lo largo del proyecto.
