...
if __name__ == '__main__':
    ...
    for img in images:
        ...
        img2 = img.copy()
        cv2.rectangle(img2,(minLoc[0]-40,minLoc[1]-40),(minLoc[0]+template.shape[1]+40,minLoc[1]+template.shape[0]+40),(0,255,0))
        cv2.imshow('img',img2)
       # cv2.imshow('template',imgfound)
        ...
        cv2.rectangle(img2,(final_rect[0],final_rect[1]),(final_rect[2]+final_rect[0],final_rect[3]+final_rect[1]),(0,0,255))
        cv2.imshow('img',img2)
        imgResize = img.copy()
        if final_rect != [0,0,0,0]:
            w = img.shape[0]*(template.shape[0]-10)/final_rect[3]
            h = img.shape[1]*(template.shape[1]-10)/final_rect[2]
            imgResize = cv2.resize(img,(h,w+40))
