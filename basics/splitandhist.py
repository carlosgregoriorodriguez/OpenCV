...
if __name__ == "__main__":
	...
	while True:		
		...
		for item,col in zip([imgB,imgG,imgR],color):
			...
			cv2.polylines(hrgb,np.array([pts],np.int32),False,col,2);
		...
		for item,col in zip([imgH,imgS,imgV],color):
			...
			hist=np.int32(np.around(hist_item));
			pts = np.column_stack((bins,hist));
			cv2.polylines(hhsv,np.array([pts],np.int32),False,col,2)
		...
		if (showHist):
			...
			cv2.polylines(original,np.array([pts],np.int32),False,(0,0,0),2)
			original = np.flipud(original);					
