...
int main( int argc, char** argv )
{
	...
	switch(select) {
		...
		case 3: {
			...
			cin >> numWasp;
			//resize(heatMap, heatMap, Size(700, 550), 0, 0, INTER_CUBIC);
             Mat heatMap(600, 600, CV_8UC3, Scalar(0));
            resize(heatMap, heatMap, Size(700, 550), 0, 0, INTER_CUBIC);
            //cambio la foto a blanco y negro
            cvtColor(heatMap, heatMap, CV_BGR2GRAY);
            ...
            for(int j=0;j<700;j++)
            {
                for (int i=0;i<550;i++)
                {
                    ...
                    heatMap.at<uchar>(Point(j, i)) = 55; //white
                    //  img[0].at<uchar>(Point(j, i)) = 240;
                }
            }
             Mat heatMap1(500, 500, CV_8UC3, Scalar(0));
			Mat tpl;
			...
			*/
			...
			for(;;)
			{
                ...
                //cada 10 frames sube 1 el valor del pixel
                for(int i=followedPoint.x-10;i<followedPoint.x+10;i++){
                    for(int j=followedPoint.y-15;j<followedPoint.y+15;j++){
                        if(numframes%10==0){
                        if(heatMap.at<uchar>(Point(j, i))<255 )
                        heatMap.at<uchar>(Point(j, i)) += 1;
                        }
                    }
                }
