...
void initialize() {
	...
	const string NAME ="lines.mpg";   // Form the new name with container
    int ex = static_cast<int>(CV_FOURCC('M','P','E','G'));     // Get Codec Type- Int form
	Size S = Size((int) roi.width,    // Acquire input size
                  (int) roi.height);
...
void detectIds() {
	...
	for(;;)
	{
		...
		resize(frame, initialImage, Size(roi.width,roi.height), roi.y, roi.x, INTER_CUBIC);
		        if(frameNum==651){
