...
# def getPoint(cont): #it is supposed that the contour is convex
# 	center,radius = cv2.minEnclosingCircle(cont)
# 	x,y = int(center[0]),int(center[1])
# 	halfR = int(radius)/2
# 	#rightP,leftP,upP,downP = [x+halfR,y],[x-halfR,y],[x,y+halfR],[x,y-halfR]
# 	points = [(x,y),(x+halfR,y),(x-halfR,y),(x,y+halfR),(x,y-halfR)]
# 	inCont = []
# 	for pt in points:
...
def getPoints(dimension,sigSquares):
	retPoints = []
...
def doAndPack(img,minArea,maxArea,direction):
	...
	copyImg = significantSQS(img.copy(),bigCont,minArea,maxArea,direction)
