...
\item Una vez delimitada la zona de interés, \textbf{es necesario
  ...
  \item \textbf{El primer paso mide el ángulo de inclinación} de la
    ...
    referencia, aplicando la \emph{Transformada de Hough} para
    rectas. La estructura ocular posee una capa con una intensidad muy
    alta denominada \emph{membrana de Bruch} en la que si se aplica
    dicha transformada, se obtiene sobre ella una recta que puede ser
    usada como referencia. Todas las líneas detectadas están
    almacenadas de forma paramétrica en una matriz de pares
...
\item Teniendo la imagen horizontal para evitar fallos debidos a la
  ...
  \item \textbf{Se somete} una copia de \textbf{la imagen} (para no
    alterar la original) \textbf{a un \emph{median Blur}} con un
    \emph{kernel} de tamaño $7$ \textbf{para eliminar el ruido
      granulado}.
...
\item \textbf{La última fase se divide en} tres grandes pasos:
  ...
  \item \textbf{Luego se determina la posición del punto inferior de
    ...
    \item Usando la función \emph{findContours} se genera un
      \emph{Canny} para encontrar los bordes, descartando los bordes
      más pequeños (ruido no eliminado por el \emph{Blur}).
